...
!_____________ Open output files
   ...
   open ( File_no(16), file = 'output_summerT.txt'      )
...
!_____________ DAILY PSYSICAL PROCESSES
   ...
   !co2atm = aco2_annual(min(year_climate+51,YearMaxCO2)) !Progress
...
!_____________ DAILY INCREMENT of LITTER AND FUEL POOLS
   ...
   flux_c_lit_RR(1) =  flux_litter_trunk + flux_litter_leaf + flux_litter_acorn + flux_litter_root + &
                       flux_litter_ag + flux_litter_bg
   
   pool_litter_leaf  = max(0.0, pool_litter_leaf  + flux_litter_leaf + flux_litter_acorn)
   ...
   pool_fuel_standT  = max(0.0, pool_fuel_standT  + flux_litter_leaf + flux_litter_acorn)
...
!  if (doy == 213) then !@ 1 Aug
      ...
      Call output_summerT(File_no(16))
   endif
...
!_____________ After Main-simulation-loop procedures
   ...
   close (File_no(16))   !output_summerT.txt

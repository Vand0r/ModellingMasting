...
!Apparent quantum yield (electron/PAR-quanta)
   ...
   (/0.1196, 0.1196, 0.1196, 0.1196, 0.1196, 0.1196, &
     ...
     0.1196, 0.1196, 0.1196, 0.1196, 0.1196,         &
     0.1196, 0.1196                                /) 
...
!Deactivation efficiencies (J mol-1)
   ...
   (/193200., 193200., 193200., 193200., 193200., 193200., &
     ...
     193200., 193200., 193200., 193200., 193200.,          &
     193200., 193200.                                     /)
...
!Local parameter
   ...
   integer,parameter::Delay_from_foliation_Q = 30 !for Quercus, PFT11
   
   !Threshold biomass that initiate reproduction (DM g)
   real,parameter::Thres_mass_reproduct = 10000.0
   
   !Quercus-PFT 11 specific parameters
   real, parameter::Fra = 0.5 !Female flower ratio
   real, parameter::Fn = 0.4 !ratio of flowers developing into acorns
   
   real  Rc !acorn mass compared to flower mass, cost of acorn production
   real  Bu !ratio of mass_available going to bucket
   real  Li !threshold limit of biomass to flower
   
   !Buffer for available resource (DM/tree)
   real,parameter::Mass_buffer = 10.0
   
   !Parameter for fall acorn timing, from leaf phenology
   integer,parameter::Days_acorn_shed = 30 !day length required for full leaf drop
...
!Local variables
   ...
   real    x, y, l, b, dif, yf     !for general usage, l for limit, b is daily bucket, dif for acorn and bucket differnce, yf female flower
   integer no, p, count   !
   integer day_until_acfall  !day until all acorn falls
...
!Open and read Rc, Li, Bu
open (unit=1,file='Rc.txt',status='old')
   read (1,*) Rc

   
open (unit=2,file='Li.txt',status='old')
   read (2,*) Li

   
open (unit=3,file='Bu.txt',status='old')
   read (3,*) Bu

! Setting counters for total and flowering Quercus PFT
If (doy == 1) then
    total_Quercus = 0.0
    flowering_Quercus = 0
end if

!Counting Quercus in spring
Do no=1, Max_no
    p=pft(no)
    
    If (dfl_leaf_onset(p) == 31) then !on day of reproduction code
        If (tree_exist(no) .and. life_type(p) == 7) then !If exists and Quercus
            total_Quercus=total_Quercus+1
            b = mass_available(no)* Bu ! percentage of mass_available going to bucket
            bucket(no) = bucket(no) + b
            mass_available(no) = mass_available(no) - b
            
            x = mass_leaf(no)+mass_trunk(no)+mass_root(no)+mass_stock(no)+mass_available(no)
            l = x* Li
            
            if (x > Thres_mass_reproduct .and. bucket(no) > l) then
                flowering_Quercus=flowering_Quercus+1
            endif
            
            !restoring original conditions
            mass_available(no) = mass_available(no) + b !Have to restore original values to avoid double taking out
            bucket(no) = bucket(no) - b !to avoid double increase
        endif
    endif
end do

!Spring part of reproduction code
Do no=1, Max_no
    p=pft(no)
    ...
    If (dfl_leaf_onset(p) == 31) then !on day of reproduction code
        If (tree_exist(no) .and. life_type(p) == 7) then !If exists and Quercus
            ...
            if (SummerT(2) == 'Cold' .and. SummerT(1) == 'Warm') then !good weather cond
            ...
            if (x > Thres_mass_reproduct .and. bucket(no) > l) then
                ...
                y = bucket(no) - l !y is flower mass

                if (SummerT(2) == 'Warm' .and. SummerT(1) == 'Cold') then !bad year
                    y = 0.5 * y
                end if
                
                yf = y * Fra !yf is female flower mass, Cf
                
                FlowNut = Fn + (flowering_Quercus/max(total_Quercus,1.0))
                if (FlowNut > 0.8) FlowNut = 0.8
                
                mass_acorn(no) = yf*Rc*FlowNut !ratio of trees flowering 
                
                bucket(no) = bucket(no) - y !flower weight removed from bucket
                
                flux_litter_acorn = flux_litter_acorn + y !flower weights
                y = 0 !flower weight spent, just to be sure
...
!Fall part of reproduction code
Do no=1, Max_no
    p=pft(no)
    day_until_acfall = Days_acorn_shed - dfl_leaf_shed(p)
    
    If (day_until_acfall == 30) then !30 days before losing leaves
                
        if (mass_acorn(no) <= bucket(no)) then
                    
            bucket(no) = bucket(no) - mass_acorn(no) !acorn mass removed from bucket
            !flux_bucket = flux_bucket + bucket(no)
            flux_litter_acorn = flux_litter_acorn + mass_acorn(no)
        else
            dif = mass_acorn(no) - bucket(no)
            
            if (mass_available(no) >= dif) then
                mass_available(no) = mass_available(no) - dif
            else
                mass_acorn(no) = mass_acorn(no) - &
                    (dif - mass_available(no)) ! real acorn mass
                mass_available(no) = 0 !depleted by acorn
            endif
                    
            bucket(no) = 0 !depleted by acorn
            !flux_bucket = flux_bucket + bucket(no)
            flux_litter_acorn = flux_litter_acorn + mass_acorn(no)
        
        endif
        
        mass_acorn(no) = 0 !added to litter
...
If (Life_type(p) == 7) then
    b = mass_available(no) * Bu ! percentage of mass_available going to bucket
    bucket(no) = bucket(no) + b ! increase of bucket
    mass_available(no) = mass_available(no) - b !daily decrease
    flux_bucket = flux_bucket + bucket(no) ! to track bucket in output
    IF (dfl_leaf_onset(p)  <= Delay_from_foliation_Q ) cycle
else if( dfl_leaf_onset(p)  <= Delay_from_foliation ) then
    cycle !before specific day from foliation
endif
...
IF ( mass_available(no) <= Mass_buffer          ) cycle !when mass available doed not exist
   ...
   !Reproduction
   x = mass_leaf(no)+mass_trunk(no)+mass_root(no)+mass_stock(no)+mass_available(no)
   If (Life_type(p) /= 7 .and. x > Thres_mass_reproduct) then    

...
!_____________ Return when this site does not have satisfies minimum-fuel-load
   ...
   Do no=1, Max_no
      ...
      x = x + Frac_trunk_AG * mass_trunk(no) + mass_leaf(no) + mass_available(no) + bucket(no)
   End do
   ...
   !mass_total(PFT_no): total above ground biomass for each PFT
      ...
      if (tree_exist(no)) then
         ...
         mass_sum(p) = mass_sum(p) + &
                       mass_leaf(no) + Frac_trunk_AG*mass_trunk(no) + mass_available(no) + bucket(no)
      end if
      End do
...
!_____________ When fire occurs
   ...
   if ( randf() <  M3(pft(no)) ) then
      !for dead trees
      mass_combust  = mass_combust + mass_leaf(no) + mass_stock(no) + mass_available(no) + bucket(no)
      mass_leaf     (no) = 0.0
      mass_stock    (no) = 0.0
...
!   End do
   ...
   if (tree_exist(no)) then
      ...
      if (randf() < prob_topkill) then
         ...
         x = mass_trunk (no) + mass_available(no) + mass_stock(no) + bucket(no)
...
!_____________ Kill assigned trees
   ...
   if (death_list(no) .or. death_list_gap(no)) then
      ...
      flux_litter_trunk = flux_litter_trunk + mass_trunk(no)+ mass_available(no)+ mass_stock(no) + bucket(no)

...
!Local parameter
   ...
   !real,parameter::Thres_mass_reproduct_Q = 1000.0 !for Quercus, PFT11
   
   !Quercus-PFT 11 specific parameters
   real, parameter::Fra = 0.5 !Female flower ratio
   real, parameter::Fn = 0.4 !ratio of flowers developing into acorns, base, later modified
...
!Local variables
   ...
   real    x, y, b, l, dif, yf, rn           !for general usage
   integer no, p, count   !
...
! Modifying Fn in Basic/W
rn = randf() * 0.4 !random number [0,0.4) 0<= and <0.4
FlowNut = Fn + rn !so Fn will range [0.4, 0.8)
...
Do no=1, Max_no
    ...
    If (dfl_leaf_onset(p) == 31) then !on day of reproduction code
        If (tree_exist(no) .and. life_type(p) == 7) then !If exists and Quercus
            ...
            if (SummerT(2) == 'Cold' .and. SummerT(1) == 'Warm') then !good year
                bucket(no) = bucket(no) + b !so bucket get an additional
                mass_available(no) = mass_available(no) - b
            end if
            ...
            if (x > Thres_mass_reproduct .and. bucket(no) > l) then
                ...
                mass_acorn(no) = yf*Rc*FlowNut !instead of pollen limitation flower to nut ratio
...
Do no=1, Max_no
    ...
    If (day_until_acfall == 30) then !30 days before losing leaves
        ...
        if (mass_acorn(no) <= bucket(no)) then
            ...
            !flux_bucket = flux_bucket + bucket(no) !bucket updated
            flux_litter_acorn = flux_litter_acorn + mass_acorn(no) !flux_acorn updated
        else
            ...
            if (mass_available(no) >= dif) then
                ...
                (dif - mass_available(no)) ! real acorn mass covered from bucket and mass_av

...
!*** 2nd line (Carbon strage & flux)
   ...
   If (tree_exist(no)) then
      c1 = c1 + mass_leaf(no)+mass_trunk(no)+mass_root(no)+mass_stock(no)+mass_available(no)+bucket(no)
   End if
   End do
...
!*** 4th line ~ (for each PFT)
   ...
   If (tree_exist(no)) then
      ...
      c_available(p) = c_available(p) + mass_available(no) + bucket(no)
   End if
   End do
...
!_____________ Prepare output variables
   ...
   if (tree_exist(no)) then
      ...
      mass_wood1   = mass_wood1   + &
                     mass_leaf(no)+mass_trunk(no)+mass_root(no)+mass_stock(no)+mass_available(no) + bucket(no)
      mass_wood2   = mass_wood2   + &
                     mass_leaf(no)+mass_trunk(no)*0.667        +mass_stock(no)+mass_available(no) + bucket(no)
...
!_____________ Write title and data ***
   if (year==1) then
   write (Fn, '(109a)', advance='no') &
   'Year, TreeDensity, TotalQ, FloweringQ, FlowNut, Heigh, DBH, BA, MassW, MassW_ag, MassG, Litter, SOM, GPP, NPP'
   write (Fn, '(79a)') &
   ' , NEE, LAImax_W, LAImax_G, LAImean_W, LAImean_G, ccon, RO, Biome, ALD, ALD_DOY'
   end if
   
   write (Fn,'( i4,a, 3(f7.1,a), f5.4, a, f7.1,a, 2(f9.3,a), 5(f6.1,a), 2(f5.1,a), f6.1,a, 4(f5.1,a), f7.3,a, f6.1,a, 2(i3,a), i3 )') &
   year                                      , ',', & ! 1 Simulation year
   tree_counter * ( (100.0/Max_loc)**2 )     , ',', & ! 2 Tree density (ha-1)
   total_Quercus * ( (100.0/Max_loc)**2 )    , ',', & ! Added total Quercus
   flowering_Quercus * ( (100.0/Max_loc)**2) , ',', & ! Added flowering Quercus
   FlowNut                                   , ',', & ! Added real Fn
...
!_____________ Write title and data ***
    if (year - Spinup_year ==1) then
    write (Fn, '(42a)') &
    'Year SummerT1 SummerAv RunningAv RunningSD'
    end if
    
    write (Fn,'( i4,3a,3(f5.2,a))') &
   year            , ' ', & ! 1 Simulation year
   SummerT(1)      , ' ', & ! SummerT
   tmp_summer_aveg , ' ', & !current summer avT
   RunningAveg     , ' ', & !past 10 years running aveg
   RunningSD                !past 10 year SD
   
END SUBROUTINE output_summerT
...
!Write data, all units except time counter are (Mg C/day/ha)
   write (Fn,'( i4, a, i3, 19(a, f8.5) )')      & 
      ...
      flux_litter_acorn            * unit_conv ,',', & !litter flux of acorn 
      flux_bucket                  * unit_conv ,',', & !total resource bucket    
...
!_____________ Prepare output variables
   ...
   do no=1, Max_no
   if (tree_exist(no)) then
      x = x + mass_leaf(no)+mass_trunk(no)+mass_root(no)+mass_stock(no)+mass_available(no)+bucket(no)
      i = i + 1
   endif
   enddo

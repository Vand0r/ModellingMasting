...
!coldest and hottest month of the year
   ...
   !Summer average temperature, initiating on Sept 1.
   
    if (Month(doy) == 9 .and. Day_of_Month(doy) == 1) then
        tmp_summer_aveg = sum(tmp_air_RunningRecord(2:93))/92.0 !(start:end) in later version
        ! and divided by real(end-start +1)
    
        !Calculate average and SD from previous 20 years
        RunningAveg = sum(tmp_summer_RunningRecord(1:20))/20.0
        RunningSD = sqrt(sum((tmp_summer_RunningRecord(1:20) - RunningAveg)**2) / 19.0)
        
        !Move SummerT records
        SummerT(2:5) = SummerT(1:4)
    
        !Decide whether warm or cold year, fill up current year
        if (tmp_summer_aveg > RunningAveg + 0.45 * RunningSD) then !upper 33% if t-distr.
            SummerT(1) = 'Warm'
        else if (tmp_summer_aveg < RunningAveg - 0.45 * RunningSD) then !lower 33%
            SummerT(1) = 'Cold'
        ...
        !Updating runningrecord
        tmp_summer_RunningRecord(2:20) = tmp_summer_RunningRecord(1:19)
        tmp_summer_RunningRecord(1) = tmp_summer_aveg
...
!_____________ Update of Ecosystem Total Carbon Pool running record
   ...
   if (tree_exist(no)) then
      y = y + mass_leaf(no) + mass_trunk(no) + mass_root(no) + mass_stock(no) + mass_available(no) + bucket(no)
   endif
   end do
...
If (doy==150) then
   ...
   if (tree_exist(no)) then
      ...
      sumup(p) = sumup(p) + mass_leaf(no) + mass_trunk(no) + mass_root(no) &
               + mass_available(no) + mass_stock(no) + bucket(no)
   end if
   End do
